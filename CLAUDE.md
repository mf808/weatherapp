...
**Netatmo resilience** (`src/datasources/netatmo.py`): `fetch()` makes 3 sequential calls (token refresh, devicelist, 6h history) with no automatic HTTP retry, against a cron-triggered display that has no backend scheduler of its own (the trigger lives entirely on the client, e.g. a cron job on the Kindle hitting `GET /` — see README "Typical usage"). A transient failure on any of the first two calls (logged via `_log_step_failure()` with the specific step + HTTP status code) falls back to `_fallback()`: a last-known-good snapshot persisted to `last_good_file` on every successful `fetch()`, reused if under `LAST_GOOD_MAX_AGE_SECONDS` (6h) old. The renderer draws a small "Netatmo-Daten von HH:MM – nicht aktuell" banner (`src/renderer.py`, keyed off a `_stale` marker in the returned dict) whenever this fallback is in use, so a stale reading is never confused with a fresh one — and `outdoor.py`/`room_climate.py`'s own missing-data defaults are `"--"`/`"--"` (not `"0.0"`/`"Gut"`) for the remaining case where there's no cache at all, so a total failure never renders as a fake real-looking reading. This was added after a real production incident: Netatmo returned `HTTPError` on nearly every 10-minute cron cycle for a ~2.5h window, and each cycle silently overwrote the last good display with zeroed-out data.
